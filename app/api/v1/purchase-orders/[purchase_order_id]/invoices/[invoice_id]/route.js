...
async function resolveAuth(req, params) {
  ...
  const url = new URL(
    `/api/v1/purchase-orders/${purchaseOrderId}/invoices/${invoiceId}`,
    backendBase,
  );
...
export async function PUT(req, { params }) {
  ...
  try {
    // JSON updates are parsed correctly by Laravel on PUT.
    ...
    // Spoof as POST + _method=PUT so Laravel receives the form fields/files.
